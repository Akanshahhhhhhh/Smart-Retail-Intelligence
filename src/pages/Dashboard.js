...
function Dashboard() {
  ...
  const [heatmapAvailable, setHeatmapAvailable] = useState(true); // NEW
  ...
  // NEW: called when the <img> fails to load (e.g. heatmap.png doesn't exist yet)
  const handleHeatmapError = () => {
  ...
  return (
    <>
      ...
      {/* UPDATED: Zone Activity Heatmap now fetches the real image from the backend */}
      <div className="heatmap">
        <h2>🗺️ Zone Activity Heatmap</h2>
        <div className="heatmap-box">
          {heatmapAvailable ? (
            <img
              src={`${API_URL}/heatmap?t=${currentTime.getTime()}`}
              alt="Customer foot-traffic heatmap"
              style={{ maxWidth: "100%", borderRadius: "8px", display: "block", margin: "0 auto" }}
              onError={handleHeatmapError}
            />
          ) : (
            <>
              <p>Heatmap generated by Agent 1 (YOLOv9 + ByteTrack) will appear here.</p>
              <p>Current Status: No heatmap found yet — run detection.py against a video first.</p>
            </>
          )}
        </div>
      </div>
